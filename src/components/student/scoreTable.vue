      ...
      this.$axios(
        ...
        if(res.data.code == 200) {
          ...
          let sujectmapVal = this.score.map((element,index) => { //通过map得到 filter:[{text,value}]形式的数组对象
            let newVal = {}
            newVal.text = element.subject
            newVal.value = element.subject
            return newVal
          })
          let sujecthash = []
          const sujectnewArr = sujectmapVal.reduce((item, next) => { //对新对象进行去重操作
            sujecthash[next.text] ? '' : sujecthash[next.text] = true && item.push(next);
            return item
          }, []);
          this.subjectfilter = sujectnewArr
        }
